{{-- video 51 solve laravel echo problem --}}
<div x-data="{
    height: 0,
    ...
    <div class="border-b flex flex-col overflow-y-scroll grow h-full">
        ...
        <header class="w-full sticky inset-x-0 flex pb-[5px] pt-[5px] top-0 z-10 bg-white border-b ">
            <div class="flex w-full items-center px-2 lg:px-4 gap-2 md:gap-5">
                ...
                @if ($selectedConversation->getReceiver()->image)
                    <a href="#" class="shrink-0">
                        <x-avatar
                            src="{{ URL('Dashboard/img/profile/users/' . $selectedConversation->getReceiver()->image->filename) }}" />
                    </a>
                @else
                    <a href="#" class="shrink-0">
                        <x-avatar src="{{ URL('Dashboard/img/profile/users/default.png') }}" />
                    </a>
                @endif
                ...
                @foreach ($loadedMessages as $key => $message)
                    ...
                    @else
                        <div class="flex justify-center">
                            <p class="text-gray-500">No messages , or replay yet</p>
                        </div>
                    @endif
                @endforeach
            @endif
                ...
                <form x-data="{ body: @entangle('body').defer }" @submit.prevent="$wire.sendMessage" method="POST" autocapitalize="off">
                    ...
                    <div class="grid grid-cols-12">
                        ...
                        <button x-bind:disabled="!body.trim()"
                            class="col-span-2 bg-blue-500 text-white rounded cursor-pointer" type='submit'>
                            Send</button>
                    </div>
                </form>
                @error('body')
