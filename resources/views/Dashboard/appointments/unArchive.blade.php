<!-- Modal -->
<div class="modal fade delete-appoint-status-modal" id="unArchive{{ $appointment->id }}" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">
                    un archive doctors</h5>
                <button type="button" class="close" data-mdb-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('appointments_admin.destroy', 'test') }}" method="post" class="deleteUpdateForm">
                {{ method_field('delete') }}
                {{ csrf_field() }}
                <div class="modal-body">
                    <h5>!! un archive {{ $appointment->name }}</h5>
                    <input type="hidden" value="2" name="delete_type">
