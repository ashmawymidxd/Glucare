<!-- Modal -->
<div class="modal fade update-password-modal" id="update_password{{ $patient->id }}" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">
                    update password {{ $patient->name }}</h5>
                <button type="button" class="close" data-mdb-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('Patients_update_password') }}" method="post" autocomplete="off" class="updatePasswordForm">
                {{ csrf_field() }}
